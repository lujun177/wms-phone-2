export default function Page() {
  ...
  const htmlContent = `
    ...
    <style>
      ...
      .log { background: #f3f4f6; padding: 8px; margin: 4px 0; border-radius: 4px; font-size: 12px; }
    </style>
    </head>
    <body>
      <div class="card">
    ...
    <script>
      const log = (msg, err) => {
        ...
        div.innerHTML += '<div class="log ' + (err ? 'error' : '') + '">' + msg + '</div>';
      };
      ...
      try {
        ...
        log('Supabase客户端创建成功', false);
      } catch(e) {
        log('客户端创建失败: ' + e.message, true);
      }
      ...
      window.searchGoods = async function() {
        ...
        if (error) {
          log('查询错误: ' + error.message + ' Code: ' + error.code, true);
      ...
      window.submit = async function(type) {
        ...
        if (error) {
          log('提交失败: ' + error.message, true);
          msg.innerHTML = '<p style="color:red">失败: ' + error.message + '</p>';
        } else {
          log(type + '成功: ' + window.currentGoods.name + ' x' + qty, false);
